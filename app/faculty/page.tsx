...
export default function FacultyPage() {
  ...
  useEffect(() => {
    async function fetchFaculty() {
      try {
        ...
        if (json.data) {
          setFacultyList(json.data as FacultyMember[]);
        } else {
  ...
  const handleMouseEnter = (e: React.MouseEvent<HTMLDivElement>) => {
    e.currentTarget.style.transform =
      "perspective(1000px) rotateX(4deg) rotateY(4deg) scale(1.02)";
  ...
  // Derive initials from a full name, e.g. "Rajesh Kumar Panda" -> "RP"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-20 section-animate">
          ...
          <p className="mt-4 text-[#42576E] text-sm sm:text-base leading-relaxed">
            Meet the experts behind every success story. Our dedicated team of
            professional educators is committed to concept mastery and
            empowering students in Berhampur, Odisha.
          </p>
        </div>
        ...
        {!loading && error && (
          <div className="text-center py-12">
            <p className="text-[#42576E]">{error}</p>
          </div>
        )}
        ...
        {!loading && !error && facultyList.length === 0 && (
          <div className="text-center py-12">
            <p className="text-[#42576E]">Faculty details coming soon.</p>
          </div>
        )}
        ...
        {!loading && !error && facultyList.length > 0 && (
          <div
            className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 section-animate"
              ...
              >
                ...
                {faculty.pic_url ? (
                  <img
                    src={faculty.pic_url}
                    alt={faculty.name}
                    className="w-20 h-20 rounded-full object-cover border border-[#7FB3E8] mb-6 group-hover:scale-105 transition-transform duration-300"
                  />
                ) : (
                ...
                <div className="space-y-1.5 text-center mb-6">
                  <p className="text-sm font-semibold text-[#42576E]">

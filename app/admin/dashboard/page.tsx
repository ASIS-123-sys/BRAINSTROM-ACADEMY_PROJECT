...
export default function AdminDashboard() {
  ...
  const [fees, setFees] = useState<(Fee & { students: { name: string; enrollment_id: string } | null })[]>([]);
  const [scores, setScores] = useState<(Score & { students: { name: string; enrollment_id: string } | null })[]>([]);
    ...
    async function loadData() {
      try {
        ...
        const [
          ...
          supabase.from("fees").select("id", { count: "exact", head: true }).neq("status", "paid"),
          supabase.from("notices").select("id", { count: "exact", head: true }),
          supabase.from("students").select("*").order("name").limit(5),
          supabase.from("faculty").select("*").order("name").limit(5),
          supabase.from("notices").select("*").order("created_at", { ascending: false }).limit(5),
          supabase.from("gallery").select("*").order("event_name").limit(5),
          supabase.from("fees").select("*, students(name, enrollment_id)").order("due_date", { ascending: true }).limit(5),
          supabase.from("scores").select("*, students(name, enrollment_id)").order("test_date", { ascending: false }).limit(5),
        ]);

        setStats([
          { label: "Total Students", value: studentsCountRes.count || 0 },
          { label: "Total Faculty", value: facultyCountRes.count || 0 },
          { label: "Pending Fees", value: feesCountRes.count || 0 },
          { label: "Notices", value: noticesCountRes.count || 0 },
        ]);
  ...
  if (loading) {
    return (
      <div className={`flex items-center justify-center min-h-[400px] ${poppins.className}`}>
        <div className="text-center space-y-3">
          <div className="w-12 h-12 border-4 border-[#06B6D4]/30 border-t-[#06B6D4] rounded-full animate-spin mx-auto"></div>
          <p className="text-sm text-[#94A3B8] font-medium animate-pulse">Loading dashboard data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={`space-y-12 ${poppins.className}`}>
      ...
      <div className="space-y-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-[#F8FAFC]">Dashboard Overview</h1>
          <p className="text-sm text-[#94A3B8] mt-1">Quick statistics for Brainstorm Academy</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pt-4">
          {stats.map((stat, i) => (
            ...
            >
              <span className="text-5xl font-extrabold text-[#06B6D4] mb-3">{stat.value}</span>
      ...
      <section id="students" className="scroll-mt-24 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-[#F8FAFC]">Students</h2>
          <Link
            href="/admin/students"
        ...
        <div style={glassCardStyle} className="p-6 overflow-x-auto hover:border-white/12 transition-all duration-300">
          <table className="w-full border-collapse text-left text-sm text-[#F8FAFC]">
            <thead>
              <tr className="border-b border-white/10 text-xs font-semibold uppercase tracking-wider text-[#94A3B8]">
                ...
                <th className="pb-3 pl-4">Phone</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {students.map((student) => (
                <tr key={student.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="py-3.5 px-4 text-[#94A3B8]">{student.enrollment_id}</td>
                  <td className="py-3.5 px-4">{student.batch}</td>
                  <td className="py-3.5 pl-4 text-[#94A3B8]">{student.phone}</td>
                </tr>
              ))}
              {students.length === 0 && (
                <tr>
      ...
      <section id="faculty" className="scroll-mt-24 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-[#F8FAFC]">Faculty</h2>
          <Link
            href="/admin/faculty"
            className="text-sm font-semibold text-[#06B6D4] hover:text-[#06B6D4]/80 flex items-center gap-1 transition-colors"
          >
            ...
            <tbody className="divide-y divide-white/5">
              {faculty.map((fac) => (
                <tr key={fac.id} className="hover:bg-white/5 transition-colors">
                  ...
                  <td className="py-3.5 pl-4 text-[#94A3B8]">{fac.experience}</td>
                </tr>
              ))}
              {faculty.length === 0 && (
                <tr>
      ...
      <section id="notices" className="scroll-mt-24 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-[#F8FAFC]">Notices</h2>
          <Link
            href="/admin/notices"
            className="text-sm font-semibold text-[#06B6D4] hover:text-[#06B6D4]/80 flex items-center gap-1 transition-colors"
          >
        ...
        <div style={glassCardStyle} className="p-6 hover:border-white/12 transition-all duration-300">
          <div className="space-y-4">
            {notices.map((notice) => (
              <div key={notice.id} className="border-b border-white/5 last:border-0 pb-4 last:pb-0">
                <div className="flex items-center justify-between gap-4">
                  <h3 className="font-semibold text-[#F8FAFC]">{notice.title}</h3>
                  <span className="text-xs text-[#94A3B8]">
                    {new Date(notice.created_at).toLocaleDateString()}
                  </span>
                </div>
                <p className="text-sm text-[#94A3B8] mt-1 line-clamp-2">{notice.content}</p>
              </div>
            ))}
            {notices.length === 0 && (
              <p className="text-center text-[#94A3B8] py-2">No notices found.</p>
            )}
          </div>
        </div>
      </section>
      ...
      <section id="gallery" className="scroll-mt-24 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-[#F8FAFC]">Gallery</h2>
          <Link
            href="/admin/gallery"
            className="text-sm font-semibold text-[#06B6D4] hover:text-[#06B6D4]/80 flex items-center gap-1 transition-colors"
          >
            ...
            {gallery.map((img) => (
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center p-3 text-center">
                  <span className="text-xs font-semibold text-[#F8FAFC]">{img.event_name}</span>
                </div>
              </div>
            ))}
            {gallery.length === 0 && (
      ...
      <section id="fees" className="scroll-mt-24 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-[#F8FAFC]">Fees</h2>
          <Link
            href="/admin/fees"
            className="text-sm font-semibold text-[#06B6D4] hover:text-[#06B6D4]/80 flex items-center gap-1 transition-colors"
          >
      ...
      <section id="scores" className="scroll-mt-24 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-[#F8FAFC]">Scores</h2>
          <Link
            href="/admin/scores"
            className="text-sm font-semibold text-[#06B6D4] hover:text-[#06B6D4]/80 flex items-center gap-1 transition-colors"
          >
            ...
            <tbody className="divide-y divide-white/5">
              {scores.map((score) => (
                <tr key={score.id} className="hover:bg-white/5 transition-colors">
                  <td className="py-3.5 pr-4 font-medium">{score.students?.name || "N/A"}</td>
                  <td className="py-3.5 px-4 text-[#94A3B8]">{score.subject}</td>
                  <td className="py-3.5 px-4">
                    <span className="font-semibold text-[#06B6D4]">{score.score}</span> / {score.total}
                  </td>
